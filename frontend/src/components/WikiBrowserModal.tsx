...
/** Read-only LLM Wiki browser (W4c) — list + markdown detail. */
export default function WikiBrowserModal({
  open,
  onClose,
}: {
  open: boolean;
  ...
  async function openPage(p: WikiPageItem) {
    try {
      const d = await api.getWikiByPath(p.path);
      setDetail(d);
    } catch (e) {
      setError(formatApiError(e));
    }
  ...
  return (
    <Modal open={open} onClose={onClose} title="LLM Wiki · 第二大脑" wide>
      <div className="flex flex-col gap-3 max-h-[70vh]">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <select
            className="bg-panel border border-edge rounded px-2 py-1 text-slate-200"
            value={kind}
          ...
          <button
            type="button"
            className="px-2 py-1 border border-edge rounded text-slate-300 hover:border-accent/50"
