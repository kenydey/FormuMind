...
- [ ] Hub 打开任意非空 wiki 页：表格与公式可读，非等宽原文墙  
- [ ] 子串搜索能过滤列表  
- [ ] 页内已有 `[[存在页]]` 可跳转；不存在页灰显  
- [ ] Chat 在 blend 开启且有命中时展示相关 Wiki 列表，点击打开与 Hub 一致的 Reader  
- [ ] 模式切换三态可切换且刷新后续回答策略（flag 关时隐藏或 no-op）

**契约 DoD（Q16）**

- [ ] 单测/组件测：wikilink 解析；模式枚举；flag 关时 Chat Wiki UI 不破坏旧路径  
- [ ] 文档：本计划 S1 勾选 + ADR 链接  
- [ ] Claims 路径回归：引用仍只来自 Raw Evidence  
- [ ] DOE：无静默硬边界变更（与现网一致）
...
1. 评审本文与 ADR（决策表已冻结于需求问答）。  
2. 开 S1 实施切片（可另写 `…-wiki-s1-impl.md` 或直接按 §4 Phase 1 开工）。  
3. **未另令前不改业务代码**——本文仅方案。
