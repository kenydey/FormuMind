...
| 选项 | 建议 |
|------|------|
| **A. Sigma + Graphology**（最贴 llm_wiki） | **推荐 P0**：生态成熟、力导向/社区插件齐；需自行实现主题色与 Hub 暗色适配 |
| B. @antv/G6 | 国内文档好；API 面大，一期够用但包体更大 |
| C. Cytoscape.js | 稳定；社区检测要自接 |
| D. 纯 SVG/Canvas 手写 | 否决（工期） |
...
| # | 任务 |
|---|------|
| 1 | 侧栏：孤立节点、断链计数、（可选）弱连通分量大小 |
| 2 | 动作：跑 Lint / 打开页 /（有 project）刷新卷宗 |
| 3 | 边权：共享邻居加权；可选 Louvain 着色 |
| 4 | `project_id` 过滤：仅 `themes/project-{id}*` 邻域 + 其链出（灰度） |
