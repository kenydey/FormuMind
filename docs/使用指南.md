...
> **2026-09 增补（见 §5.21–§5.25）**：知识库 Hub（质量运营 / Wiki / 卷宗）、推荐「为何推荐」与约束追踪、手工供应报价与 `stale_price`、台账项目级自动闭环与预测软校准、持久化 hybrid 检索 vs 会话 FAISS。
...
**三大 legacy 产品域**（另有第四域 `autodeposition_coating` 自沉积涂料）：
...
| `autodeposition_coating` | 自沉积涂料 | 膜重、耐盐雾、成本 / VOC |
...
| RAG（会话聊天） | ColBERT/PyLate（GPU）· **BM25+FAISS**（CPU）· sentence-transformers | 内存 TF-IDF |
| RAG（持久化知识库） | **hybrid BM25 + 向量余弦**（`document_chunks`，`/api/kb/hybrid-search`）；近 scan 上限或 p95 过热时 BM25 预筛 | 无嵌入时关键词 |
...
- **右栏（操作）**：顶部 **PathWizard** 引导三条主路径（配方 / 替代 / 知识库）。其下按钮各自打开聚焦弹窗 —— 🧪 技术需求（含**约束追踪**：哪些 brief 字段进了评分/DOE）、⭐ 推荐配方（卡片可展开**为何推荐**）、🎯 逆向设计、🔁 材料替代、🔬 DOE、📋 **实验台账**（项目级「保存后自动闭环」「预测偏差软校准」勾选，默认关）、📄 检测报告、📈 寻优、⚙️ 工艺、🔄 自驱动闭环、📚 **知识库 Hub**（资料归档 · 检索探针 · 质量运营 · Wiki · 图谱 · 卷宗报告）。
...
- **语义 RAG** —— 安装 `sentence-transformers`（`embedding` extra）后，**会话**聊天检索可走嵌入（BM25+FAISS / TF-IDF 回退）。**持久化知识库**另走 `hybrid_search`（BM25 + 余弦，见 §5.21），**不是** chromadb，也与会话 FAISS **不是同一条路径**。
...
`GET /api/kg/contradictions?entity_id=` 或 `?q=` 比较实体的**文献语义关系**（`substitutes`/`synergizes`/`inhibits`/…）与 `kg_feedback` 写回的**团队实测** `measured_performance` 边：当文献声称某材料可替代/协同，但团队实测该领域性能偏低，返回 `contradiction_flag` 标记（`substitute_vs_poor` / `synergy_vs_poor` / `inhibit_vs_good` 等），强度 = |实测偏离| × 文献置信度，超过 `FORMUMIND_KG_CONTRADICTION_THRESHOLD`（默认 0.3）才标记。

- 前端 `KgRelationPanel` 在替代候选行展示 **⚠ 实测反驳** 徽标，折叠态汇总「N 处矛盾」。
- `discover_substitutes` 对冲突候选**置后排序**（`FORMUMIND_KG_CONTRADICTION_DEMOTE=true` 时），让实测反驳的替代物不抢占推荐位。
- `GET /api/kg/feedback/report` 的 `recent_bias` 从各 campaign `loop_history` 抽取最近 RMSE 趋势（best-effort）。
- 纯只读视图，**不修改**已有关系边；无实测实体返回空列表。
...
原材料库是**种子库 + 数据库叠加层**，并以普通 Mapping 暴露。种子化学数据优先；种子没有的字段——`availability`、`supplier`、`functional_class`、`hansen_*`、`substitute_group`——由数据库补齐。

**手工供应（A′）**：材料详情可编辑供应商行的 `price_cny_per_kg`、`price_observed_at`、`lead_time_days`、`moq`、`pack_size`（`price_source=manual`）。观测日过旧或有价无日期时标注 **`stale_price`**。  
推荐解释的「供应风险」与材料替代候选会读取这些字段（徽章 / 轻降权）；**默认不把 stale 价狠扣进总分**。成本预测目前仍主要参考目录工程估价（报价进 `cost_cny_per_kg` 已列入后续 backlog）。

| 端点 | 用途 |
|---|---|
| `GET /api/materials` | 检索材料库（可按角色、化学族、可互换组过滤） |
| `POST /api/materials` | 新增或更新材料（可含 `suppliers_json` 手工报价） |
| `POST /api/materials/availability` | 标记 `available` / `restricted` / `discontinued` |

> 归一化表 `suppliers` + `material_suppliers` 为权威存储；`materials.suppliers_json` 默认仍 dual-write 兼容，读路径会 hydrate。
...
右栏 **📚 知识库** 打开 Knowledge Hub（与左栏「本次会话资料」互补）：

| 页签 | 作用 |
|------|------|
| **资料** | 持久化 source / chunk；软归档、恢复；归档后默认不参与检索 |
| **检索探针** | `/api/kb/query-test`：BM25 / cosine / hybrid 分项，调 `kb_hybrid_alpha` |
| **质量运营** | `/api/kb/quality-ops`：启发式 quality 分、scan 压力、topicality 影子/真闸统计、**hybrid p50/p95**、近上限 CTA |
| **Wiki** | 编译记忆页（entities/concepts/…）；摘要可嵌入双轨检索（`wiki_embed_enabled`，Claims 仍滤 wiki） |
| **图谱** | Wiki 页 wikilink 图（≠ 材料 KG / Neo4j） |
| **报告 / 卷宗** | 项目 dossier 确定性表格 + 可选叙述；STORM 长文（旗标默认偏关） |

**质量闸（默认）**：主题相关性真闸开启（`kb_relevance_shadow=False`）；归档 retention purge **永不自动物理删**，需 dry-run + 确认。

**检索路径务必分清**：

| 路径 | 用途 | 实现 |
|------|------|------|
| 会话 RAG | 中栏聊天 | `rag.BM25FAISSStore` 等 |
| 持久化 KB | Hub / 推荐融合 | `hybrid_search` 扫 `document_chunks`（scan_limit 默认 5000；近上限或 p95 热时 BM25 预筛子集再算余弦） |

### 5.22 推荐解释与约束追踪

每次经 `_score_and_validate` 的配方会附带 `Formulation.explain`：

- **目标命中 / 约束未满足**、证据引用、KG 相容/协同/抑制、**供应风险**、不确定性、是否 **偏差校准**
- **约束追踪**（`effect_trace`）：objectives / levers / VOC 等哪些 **生效**、哪些 **仅展示**（如部分 `ph_target` / 膜重标量）

前端：排行榜卡片「为何推荐」；技术需求面板下也有约束追踪芯片。改一个 objective 后重新推荐，hit/miss 会跟着变。

### 5.23 实验台账飞轮开关（默认关）

**📋 实验台账** 工具条（项目工作区持久化，**非**全局默认开）：

| 开关 | 行为 |
|------|------|
| **保存后自动闭环** | Completed+测量行 sync 后触发 optimize + 下一轮 DOE 建议；**不会**自动写 next_doe 到台账（需另开自动采纳 DOE） |
| **本项目卷宗事件自动更新** | 白名单事件刷新 dossier 对应节 |
| **本项目预测偏差软校准** | 用台账 `prediction_bias.mean_error` 校正 **predicted**（`predicted−mean_error`）；**不改** measured / Claims / DOE；榜卡显示「已校准」 |

另有 **BiasTrend** 面板与 `loop_status`（running / converged / paused / failed）。全局 env 仍可关；项目 OR 全局才生效。

### 5.24 PathWizard 与旗标成熟度

右栏顶部 **PathWizard** 三选一：配方推荐、材料替代、知识库治理。Settings / 依赖旗标带 `maturity`（stable / beta / experimental），避免把实验能力当成默认主路径。

### 5.25 主路径默认（产品约束备忘）

以下**默认关闭**，需显式 env 或项目勾选：

- `auto_loop_on_sync` / `auto_adopt_next_doe_on_loop` / `prediction_bias_soft_correct`
- `kg_relations_on_ingest`、归档 TTL 物理 purge、停 `suppliers_json` dual-write
- Neo4j / 外部向量库（Qdrant 等）——未到度量阈值不默认引入
...
| GET | `/api/experiments/workbench/{id}` | 读取 AG Grid 台账行（含 `loop_status`） |
| PUT | `/api/experiments/workbench/sync` | 批量保存台账；可选触发项目/全局 auto_loop |
| GET | `/api/experiments/workbench/{id}/bias-trend` | prediction_bias 趋势（决定是否开软校准） |
| GET | `/api/kb/stats` | 活跃/归档源与 chunk、scan 压力 |
| GET | `/api/kb/quality-ops` | Hub 质量运营聚合（含 hybrid p50/p95） |
| POST | `/api/kb/hybrid-search` | 持久化 KB hybrid 检索 |
| POST | `/api/kb/query-test` | 检索探针（分项分数） |
| POST | `/api/kb/retention/purge` | 归档源 retention（默认 dry-run） |
| GET/POST | `/api/wiki/…` | Wiki 页 / 图 / dossier / STORM（旗标控制） |
...
**内网 / 实验室**：在 `.env` 中设置 `FORMUMIND_API_AUTH_ENABLED=false`，设置页无需平台令牌即可加载。**公网部署**：保持默认 `true`，设置 `FORMUMIND_API_TOKEN`，并在设置页「API 访问令牌」填写相同值（运行时保存，勿打进前端镜像）。
...
| `FORMUMIND_AUTO_LOOP_ON_SYNC` | `false` | 台账保存后自动闭环（也可项目勾选 OR） |
| `FORMUMIND_AUTO_ADOPT_NEXT_DOE_ON_LOOP` | `false` | 闭环后自动采纳下一轮 DOE（实验性，默认关） |
| `FORMUMIND_PREDICTION_BIAS_SOFT_CORRECT` | `false` | 预测偏差软校准（也可项目勾选 OR） |
| `FORMUMIND_KB_SEARCH_SCAN_LIMIT` | `5000` | 持久化 hybrid 扫 chunk 上限 |
| `FORMUMIND_KB_HYBRID_ANN_GATE_P95_MS` | `800` | hybrid p95 超阈则启用 BM25 预筛 |
| `FORMUMIND_KB_RELEVANCE_SHADOW` | `false` | `false`=主题相关性真闸；`true`=仅影子统计 |
| `FORMUMIND_MATERIALS_SUPPLIERS_JSON_DUAL_WRITE` | `true` | 材料供应商 JSON 双写（兼容窗） |
| `FORMUMIND_WIKI_ENABLED` / `WIKI_EMBED_ENABLED` / `WIKI_PAGE_GRAPH_ENABLED` | 见 `.env.example` | Wiki 编译记忆 / 嵌入双轨 / 页图 |
...
| `FORMUMIND_PDF_LOCAL_OCR` | `false` | 本地版面解析（pymupdf4llm）内置的逐页 OCR。⚠️ **库默认是开启的**，而这一层跑在解析级联最前面、对每一份 PDF 都跑，机器上装了 Tesseract 就每页先 OCR 一遍。实测 6 页图片型 PDF：开 16.7 秒 / 关 0.8 秒。扫描件不受影响——没有文字层的文档会被识别出来，交给 RapidOCR 或 MinerU。**混合文档**（少量文字页 + 多页扫描）不算「扫描件」（判定是逐页取 `all()`），其无文字层的页面改由单页升级到 MinerU 处理；未开 MinerU 时会按文档记一条 warning 说明有几页读不出来 |
| `FORMUMIND_MINERU_PAGE_TIMEOUT_S` | `90` | **单页**升级的等待上限（秒）。逐页升级是串行的，超时逐页累加，所以比整份文档的额度短得多；网络不通时调大它只会把等待翻倍 |
| `FORMUMIND_MINERU_TIMEOUT_S` | `300` | **整份**扫描件送 MinerU 的等待上限（秒） |
| `FORMUMIND_MINERU_MAX_PAGE_FAILURES` | `3` | 连续多少页升级失败就放弃这份文档的升级（`0` = 不熔断）。没有它，一条不通的网络会被逐页重试，同一个错误付满 20 次超时 |
...
- **SQLite 使用 WAL 日志模式**（每次连接自动设置）。默认的 `delete` 模式下读写互斥 ——
  一次长时间的入库事务会把所有无关的读请求堵在后面，这就是生产环境里「database is
  locked」的来源；WAL 允许任意多个读者与一个写者并发。
  ⚠️ **备份时要连同 `formumind.db-wal` 和 `formumind.db-shm` 一起拷贝**：已提交但尚未
  checkpoint 的数据在 `-wal` 文件里，只拷 `.db` 会丢数据。
  内存库与网络文件系统不支持 WAL，此时自动保持原有模式（不影响启动）。
...
pip install -e ".[embedding]"    # sentence-transformers → 语义嵌入（会话 RAG / KB hybrid）
