...
- **左栏（资料来源）**：顶部是**研究主题提示词框**（为检索与问答提供上下文）；其下是来源类型复选框（专利 / 文献 / 互联网 / 本地文件 / **📓 NotebookLM**）、文件上传按钮、**检索**按钮，以及已加载资料列表（每条可删除）。
...
> 离线时专利检索返回精选种子语料；文献、互联网、NotebookLM 检索需安装对应可选 extra（见 §12）。
