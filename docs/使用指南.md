...
> 本文档对应 FormuMind v0.3：仿 NotebookLM 的三栏重设计（资料来源 / 研究 / 操作）、九家大模型供应商支持与应用内设置对话框、多源检索（专利 / 文献 / 互联网）、基于 markitdown 的本地文件上传，以及 RAG 接地问答 —— 在 v0.2 功能集（多目标寻优、成本/可持续性、置信区间、DOE 导入导出、SQL 持久化、配方导出、收敛图、模型仪表盘、历史会话）之上构建。
