...
- **左栏（资料来源）**：顶部是**研究主题提示词框**（为检索与问答提供上下文）；其下是来源类型复选框（专利 / 文献 / 互联网 / 本地文件 / **📓 NotebookLM**）、文件上传按钮、**检索**按钮，以及已加载资料列表（每条可删除）。每个来源类型复选框旁紧靠一个**状态指示点**：🟢 绿色 = 在线库已安装可用，🟡 黄色 = 离线回退（专利永远有精选种子语料），🔴 红色 = 对应 pip 库未安装。「检索」按钮下方还有 **🔬 深度研究** 按钮，点击后触发 `POST /api/research/deep`，运行 KnowledgeCohort 多智能体管线（web_agent + HyDE + 重排 kb_agent + 带强制引用的 report_agent），以异步任务方式执行，结果自动推送至中栏。勾选「文献」或「互联网」时，来源面板会出现 **🧪 ChemCrow 徽章**，显示 ChemCrow 化学增强检索（SerpAPI + paper-qa）是否已启用。检索失败时（如后端未启动），左栏顶部显示**红色错误横幅**并附带后端启动提示。
- **中栏（研究）**：基于**已加载资料**进行问答的对话界面（语义嵌入或 TF-IDF 重排 → LLM 作答），每条回答下方以引用标签标注所用证据。
- **右栏（操作）**：六个按钮各自打开一个聚焦的**弹窗** —— 🧪 技术需求（顶部含 **✨ 智能解析** 一键填表）、⭐ 推荐配方（AI 推荐 Top-N + 每张卡的 🔍 **IP 合规分析**）、🔬 DOE 设计（5 种设计 + 🧠 AI 主动选点、填实测、回灌训练、模型仪表）、📈 寻优收敛（贝叶斯闭环、收敛折线图）、⚙️ 工艺优化（固化温度/时间、分散转速、膜厚、浴温、pH 等）、🔄 自驱动闭环（数据 → 重训 → 寻优 → 下一批主动学习 DOE 一键完成）。按钮上的状态徽章显示运行中 / 结果条数。
...
1. 在 **🔬 DOE 设计** 弹窗，选择设计类型（中心复合 CCD、Plackett-Burman、…，或 **🧠 AI 主动选点**），点击**生成 DOE**。AI 主动选点行以紫色高亮 —— 这些是期望改善（EI）选出的最具信息量的实验点。
2. 系统生成实验记录表，每行一个实验，列出各因子的自然值 + 一列空白"实测"。
3. 两种回灌方式：
   - **手动**：在"实测"列直接填入实验室测得的指标值，点击 **③ 回灌实验结果并训练模型**。
   - **批量**：点**导出 CSV** 把表发给实验室 → 填好后点**导入 CSV** 上传。
4. 当某 (域, 指标) 累积样本 ≥ `min_train_samples`（默认 4），平台自动训练数据驱动模型。

**结果**：模型质量仪表盘显示 R² 半圆仪表 + RMSE；推荐与寻优切换为"经验 + 实测"混合预测。
...
- **寻优分层** —— 闭环寻优器自动选择已安装的最强引擎：**BoTorch**（GP + qNEHVI，`bo` extra）→ **Summit**（Bayesian/TSEMO，`heavy` extra）→ **Optuna**（NSGA-II/TPE，纯 CPU，轻量 `optimize` extra）→ 内置 numpy UCB 寻优器。所用 `engine` 会写入寻优结果。
- **主动学习 DOE** —— 当 ≥ `min_train_samples` 条记录存在时，DOE 弹窗的 *🧠 AI 主动选点* 用已训练代理模型在 DOE 网格上算期望改善；否则回退到随机 LHS。
...
- 每次完成**研究、寻优、回灌、闭环**后，平台自动保存一份会话快照（需求 + 排行榜 + 模型 + 收敛曲线 + 时间戳）。
