...
| `FORMUMIND_PDF_LOCAL_OCR` | `false` | 本地版面解析（pymupdf4llm）内置的逐页 OCR。⚠️ **库默认是开启的**，而这一层跑在解析级联最前面、对每一份 PDF 都跑，机器上装了 Tesseract 就每页先 OCR 一遍。实测 6 页图片型 PDF：开 16.7 秒 / 关 0.8 秒。扫描件不受影响——没有文字层的文档会被识别出来，交给 RapidOCR 或 MinerU |
