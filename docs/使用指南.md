...
> ⚠️ **`patent-client` 会降级两个基础依赖**：它要求 `httpx<0.28` 与 `pypdf<5.0`，而本项目钉的是 `httpx==0.28.1` / `pypdf==6.14.2`。`pip install -e ".[intel]"` **不会报错，而是静默降级**这两个包。若只是要专利检索则无需安装它——专利全文现在走 Google Patents 落地页。
