...
FLAG_REGISTRY: tuple[EnvFlag, ...] = (
    ...
    EnvFlag("chat_cross_encoder_enabled", "Chat Cross-Encoder",
            "问答主路径在会话 BM25 召回后可选 CE 精排；默认关；超时/失败回退 BM25 顺序。",
            "retrieval", "需 embedding extra"),
    EnvFlag("wiki_storm_claim_regenerate", "STORM 论断定向再生",
            "claim_check 判定 needs_regenerate 时对失败章节再生一轮（仍 draft_not_claims）。默认关。",
            "wiki"),
    EnvFlag("wiki_storm_section_rerank", "STORM 章节 CE 精排",
            "章节证据池可选 cross-encoder；默认关。",
            "wiki", "需 embedding extra"),
